...
class Object extends CActiveRecord
{
        ...
        protected function afterSave()
	{
        ...
        public static function getTags($object_id){
                $req = Yii::app()->db->createCommand(
                                    "SELECT t.name 
                                    FROM tt_tag t, tt_tag_relationships r, tt_object o
                                    WHERE t.id = r.tag_id 
        ...
        public static function getRelatedContentByTags($id, $max) {
            ...
            $criteria->join = 'join tt_tag_relationships ft on ft.object_id = t.object_id';
		...
		/**  Get all active pages for feed back */
		
		 public static function getParentClasses($render=true,$object_id=null){                        
                if($object_id!==null){
                    $objects=Object::model()->findAll('object_status = :status and object_id <> :pid',array(':status'=>  ConstantDefine::PAGE_ACTIVE,':pid'=>$object_id));                
                }
                else {
                    $objects=Object::model()->findAll('object_status = :status',array(':status'=>  ConstantDefine::PAGE_ACTIVE));                
                }
                ...
                if($objects && count($objects) > 0 ){
                    $data=CMap::mergeArray($data,CHtml::listData($objects,'object_id','object_name'));                        
		 ...
		 public static function getParentObject($render=true,$object_id=null){                        
                if($object_id!==null){
                    $objects=Object::model()->findAll('object_status = :status and object_id <> :pid and object_parent = :op',array(':status'=>  ConstantDefine::PAGE_ACTIVE,':pid'=>$object_id,':op'=>0));                
                }
                else {
                    $objects=Object::model()->findAll('object_status = :status and object_parent = :op',array(':status'=>  ConstantDefine::PAGE_ACTIVE,':op'=>0));                
                }
        ...
        public function checkTags($attribute,$params)
	{
        ...
        public static function extraLabel(){
			...
			'object_date' => t('Page Date'),
			'object_date_gmt' => t('Object Date Gmt'),
			'object_content' => t('Content'),
			'object_title' => t('Meta Title'),
			'object_excerpt' => t('Meta Tags(extra)'),
			'object_status' => t('Active Status'),
			'comment_status' => t('Review Allowed'),
			'allow_enquiry' => t('Enquiry Allowed'),
			'allow_reserve' => t('Reservation Allowed'),
			'object_password' => t('Object Password'),
			'object_name' => t('Page Name'),
			'start_date' => t('Start Date'),
			'end_date' => t('Ending With'),
			'object_modified' => t('Page Modified'),
			'object_modified_gmt' => t('Object Modified Gmt'),
			'object_content_filtered' => t('Object Content Filtered'),
			'object_parent' => t('Page Parent'),
			'guid' => t('Guid'),
			'object_type' => t('Object Type'),
			'comment_count' => t('Comment Count'),
			'allow_index' => t('Allow Search Engine to Index'),
            'allow_follow' => t(' to Follow'),
			'object_slug' => t('Page URL'),
			'object_description' => t('Meta Description'),
			'object_keywords' => t('Meta Keywords'),
			'lang' => t('Language'),
			...
			'object_googlecode' => t('Google Analytics Code'),
			'object_bingcode' => t('Bing Analytics Code'),
			'object_h1' => t('Page H1 Tag'),
			'object_h2' => t('Page H2 Tag'),
			'object_mainmenu' => t('Menu Name'),
			'object_footermenu' => t('Footer Menu'),
			'object_breadcrumbs' => t('BreadCrumbs'),
			'object_canonical' => t('Canonical URL'),
			'offer_desc' => t('Offer Description'),
		...
		$sort = new CSort;
			...
			$criteria->join="JOIN tt_transfer as tr ON t.object_id = tr.object_id
			LEFT OUTER JOIN tt_transfer tr2 ON (t.object_id = tr2.object_id AND 
