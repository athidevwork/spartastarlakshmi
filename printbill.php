...
$pdf->Cell(3, 0.25, ": ".$billnumber, '0', "L");
$pdf->SetFont('arial','',12);
...
$pdf->Ln(.3);
	...
	include("config_db2.php");
	$query5 = "select * from fees_details where patient_id='$pid' and paid_status ='1' AND ip_id='' and bill_number='$billnumber'";
	$res5 = mysql_query($query5);
	//$i=1;
			if(mysql_num_rows($res5) != 0){
$pdf->SetFont('arial','B',10);
...
$pdf->Ln(.3);
	...
	$pdf->Ln(.3);
	$pdf->SetFont('arial','B',12);
	$pdf->Cell(5.1, 0.25, 'Total Amount (T)(CB+OB)  :', '0','0', 'R');
	$pdf->SetFont('arial','',12);
	$pdf->Cell(1, 0.25, number_format($fees_res,2), '0', 'R');
	$pdf->Ln(.3);
	$pdf->SetFont('arial','B',12);	
	$pdf->Cell(5.1, 0.25, 'Amount Paid (P) :', '0','0', 'R');
	$pdf->SetFont('arial','',12);
