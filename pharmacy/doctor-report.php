    ...
    <div class="main-content-inner">
      <div class="page-content">
        <div class="page-header">
            <h1> Report <small> <i class="ace-icon fa fa-angle-double-right"></i> Doctor Report </small> </h1>
          </div>
        <!-- /.page-header -->
        <div class="row">
          <div class="col-xs-12">
			...
			$query=mysql_query("select id,doctorname from tbl_doctor");
			?>
					<label for="name" class="col-xs-2 control-label">Doctor</label>
                    <div class="col-xs-2">
                      <select class="form-control" name="docname" id="docname">
...
<thead>
  <tr>
    ...
    <th style="text-align: center;">Bill paid</th>
...
function rptPrintXLS(){
	var d1 = $('#dtfrom').val(), d2 = $('#dtto').val(), pm = $('#docname').val();
	if(d1 == "")
		return false;
	var data = "fromdate="+d1+"&todate="+d2+"&doc="+pm;
	window.open("manage-report/excel-doctor-report.php?"+data)
}
function rptDisplay(){
	var d1 = $('#dtfrom').val(), d2 = $('#dtto').val(), pm = $('#docname').val();
	...
	$.ajax({
		type: 'post',
		url: 'manage-report/return-doctor-report.php',
		data: {
			fromdate: d1,
			fromto: d2,
			doc: pm,
		...
		success: function(msg) {
			...
			for(var i = 0; i < x.length - 1; i++){
				var tr = "<tr><td>"+x[i].date+"</td><td>"+x[i].billno+"</td><td>"+x[i].drname+"</td><td>"+x[i].name+"</td><td class='tax1'>"+x[i].totalamt+"</td></tr>";
				 $(table).append(tr);
			}
			var tr = "<tr><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th style='text-align: right;'>Total</th><th style='text-align:center;'>"+x[i].tamt+"</th></tr>";
			$(table).append(tr);
		}
	});
}
