...
<?php
	...
	if($_SESSION['prep'] == 1 || $_SESSION['srep'] == 1){
		echo '<li class="active open hover highlight"> <a href="#" class="dropdown-toggle"> <i class="menu-icon fa fa-calendar"></i> <span class="menu-text"> Reports</span> <b class="arrow fa fa-angle-down"></b> </a> <b class="arrow"></b><ul class="submenu">';
    ...
    <div class="main-content-inner">
      <div class="page-content">
        <div class="page-header">
            <h1> Report <small> <i class="ace-icon fa fa-angle-double-right"></i> Availability Report </small> </h1>
          </div>
        <!-- /.page-header -->
        <div class="row">
          <div class="col-xs-12">
...
function rptDisplay(){
	var d1 = $('#dtfrom').val(), d2 = $('#dtto').val();
	var table = $("#tbl-adj-rpt > tbody");
	if(d1 == "")
		return false;
	$.ajax({
		type: 'post',
		url: 'manage-report/return-avl-report.php',
		data: {
			fromdate: d1,
			fromto: d2,
			//paymentmode: pm,
		...
		success: function(msg) {
			...
			for(var i = 0; i < x.length - 1; i++){
				var tr = "<tr><td>"+x[i].datentime+"</td><td>"+x[i].productname+"</td><td>"+x[i].batchno+"</td><td>"+x[i].qty+"</td><td>"+x[i].expiry+"</td><td>"+x[i].expiry+"</td><td>"+x[i].adjreason+"</td><td>"+x[i].adjtype+"</td></tr>";
				 $(table).append(tr);
			}
