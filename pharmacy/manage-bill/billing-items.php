...
//		$desc1 =  substr($rs['stocktype'],0,3) . '. ' . $rs['productname'];
		$unit = $rs['unitdesc'];
		$sql = mysql_query("SELECT qty,id,aval,mrp,vat / qty as vatval FROM tbl_purchaseitems WHERE status = 1 AND productid = $code AND batchno = '$batch' AND expirydate = '$expiry'");
		...
		if(mysql_query($sql)){
			...
			$array = array("code"=>$code,"desc"=>$desc,"qty"=>$qty,"batch"=>$batch,"expi"=>$exp,"amt"=>$amount,"id"=>$pid,"vatval"=>$vatval);
