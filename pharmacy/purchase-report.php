    ...
    <div class="main-content-inner">
      <div class="page-content">
        <div class="page-header">
            <h1> Report <small> <i class="ace-icon fa fa-angle-double-right"></i> Purchase Report </small> </h1>
          </div>
        <!-- /.page-header -->
        <div class="row">
          <div class="col-xs-12">
...
function rptDisplay(){
	...
	$.ajax({
		type: 'post',
		url: 'manage-report/return-purchase-report.php',
		data: {
			fromdate: d1,
			fromto: d2,
			paymentmode: pm,
		...
		success: function(msg) {
			...
			for(var i = 0; i < x.length - 1; i++){
				var tr = "<tr><td>"+x[i].date+"</td><td>"+x[i].billno+"</td><td>"+x[i].supplier+"</td><td>"+x[i].type+"</td><td>"+x[i].pending+"</td><td>"+x[i].duedate+"</td><td class='tax1'>"+x[i].paidamt+"</td><td>"+x[i].paidto+"</td></tr>";
				 $(table).append(tr);
			}
			var tr = "<tr><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th style='text-align: right;'>Total</th><th style='text-align:center;'>"+x[i].tamt+"</th></tr>";
			$(table).append(tr);
		}
	});
}
