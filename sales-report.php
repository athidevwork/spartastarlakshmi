	...
	$.ajax({
		...
		success: function(msg) {
			...
			for(var i = 0; i < x.length - 1; i++){
				var tr = "<tr><td>"+x[i].date+"</td><td><a href='printbill.php?billno="+x[i].billno+"' target='_blank' style='text-decoration:none;'>"+x[i].billno+"</a></td><td>"+x[i].patientname+"</td><td>"+x[i].drname+"</td><td>"+x[i].paymentmode+"</td><td class='tax1'>"+x[i].totalamt+"</td></tr>";
				 $(table).append(tr);
			}
