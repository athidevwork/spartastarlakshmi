	...
	function get_lab_det1($lab_det,$lab_det1)
{
  ...
  include("config_db2.php");
			 $sql1 = "SELECT * FROM inv_patient WHERE patientid='$uid' AND pat_ip_status=0";
...
$pdf->Ln(.4);
$pdf->MultiCell(6.3, 0.25, "IP BILL DETAILS" , '0', "C");
$pdf->Ln(.3);
...
function get_labtype($lab_id_arg){
	...
	$query5 = "select fees_amount from room_bill_details where bill_number='$bill_number'";
...
$pdf->SetFont('arial','',10);
	$pdf->Cell(.6, 0.25, '', '0', "C");
$pdf->Cell(.5, 0.25,$i++, '0', 'C');
$pdf->Cell(4, 0.25, 'OT/ Procedure Charges', '0', 'C');
//$pdf->Cell(1, 0.25, '' ,'0', "C");
//$pdf->Ln(.3);
...
//$pdf->Ln(.3);
	...
	/*CONSULTANT FEES*/
	include("config_db2.php");
   $query5 = "select fee from consultant_details where patient_id='$patientid' and  bill_number='$bill_number'";
...
$pdf->SetFont('arial','',10);
	$pdf->Cell(.6, 0.25,'', '0', "C");
$pdf->Cell(.5, 0.25,$i++,'0', 'C');
$pdf->Cell(4, 0.25, 'Consultant Fees', '0', 'C');
//$pdf->Cell(1, 0.25, '' ,'0', "C");
//$pdf->Ln(.3);
...
//$pdf->Cell(.5, 0.25, $i++, '0', 'C');
//$pdf->SetFont('arial','',10);
//$pdf->Cell(4, 0.25, get_dept_name1($rs5['department']).'-'.get_consult_name1($rs5['consultant']).'-'.get_visit_name1($rs5['visit']), '0', 'C');
//$pdf->SetFont('arial','',12);
//$pdf->Cell(1, 0.25, $rs5['fee'], '0', "C");
//$pdf->Ln(.3);
	
	 $fees6+=$rs5['fee'];
}
$pdf->SetFont('arial','',12);
	$pdf->Cell(1, 0.25, number_format($fees6,2),'0', "C");
	$pdf->Ln(.3);
		}	
/*OTHER CHARGES*/
	include("config_db2.php");
...
$pdf->SetFont('arial','B',10);
	$pdf->Cell(.6, 0.25, '', '0', "C");
$pdf->Cell(.5, 0.25, '', '0', 'C');
$pdf->Cell(4, 0.25, 'PHYSIOTHEROPY-BILL', '0', 'C');
$pdf->Cell(1, 0.25, '' ,'0', "C");
$pdf->Ln(.3);
...
//$pdf->AddPage();
			...
			$fees_tot=$fees2+$fees5+$count2+$count+$fees6+$countroom;
...
/*ROOM CHARGES*/
	...
	$pat_ip_id = get_pat_ip_id($patientid);
	$query5 = "select * from room_bill_details where bill_number='$bill_number'";
	$res5 = mysql_query($query5);
	$i=1;
			if(mysql_num_rows($res5) != 0){
$pdf->SetFont('arial','B',10);
...
$pdf->SetFont('arial','',8);
$pdf->Cell(1.1, 0.25, '', '0', "C");
$pdf->Cell(4, 0.25, 'Date:('.date("d-M-Y   h:i:s",$created_date['Date']).')', '0', 'C');
//$pdf->Cell(4, 0.25, 'Date:('.$rs5['created_date'].')', '0', 'C');
//$pdf->Cell(4, 0.25, 'Date:('.$created_date->format('Y-m-d').')', '0', 'C');
//$pdf->Cell(4, 0.25, 'Date:('.date("d-M-Y   h:i:s",$created_date['Date']).')', '0', 'C');
//$pdf->Cell(4, 0.25, "Date : ".date("d-M-Y " ,strtotime($created_date)), '0', 'C');
$pdf->Cell(1, 0.25, '' ,'0', "C");
$pdf->Ln(.3);
...
$pdf->Ln(.3);
	...
	/*CONSULTANT FEES*/
	include("config_db2.php");
   $query5 = "select * from consultant_details where patient_id='$patientid' and  bill_number='$bill_number'";
	$res7 = mysql_query($query5);
	//$i=1;
	$fees6 =0;
		if(mysql_num_rows($res7) != 0){
